...
export const CarruselEntes = () => {
  ...
  const handlePrev = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === 0
        ? Math.max(0, infoEnte.length - itemsPerPage)
        : prevIndex - 1
    );
  };
  ...
  const handleNext = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex + itemsPerPage >= infoEnte.length ? 0 : prevIndex + 1
    );
  };
      ...
      <div className="bloqueZonaEnte">
        ...
        <div className="carousel w-full contenedores-informacion">
          {infoEnte
            ...
            .map((ente: any) => (
              <div
                key={ente.id}
                className="carousel-item relative w-full carousel-item-ente"
              >
                {userInfo && userInfo.user.rol.id === 1 && (
                  <FaTrash
                    title="Eliminar"
                    className="trash-ente"
                    onClick={() => handleClickOpenDelete(ente.id)}
                  />
                )}
                {userInfo && userInfo.user.rol.id === 1 && (
                  <FaEdit
                    title="Editar"
                    className="edit-ente"
                    onClick={() => handleOpenEditInfo(ente.id)}
                  />
                )}
        ...
        >
          ...
          <DialogActions className="justify-center pb-6">
            ...
            <Button onClick={handleClsAddInfo} variant="outlined" color="error">
              Cancelar
            </Button>
          </DialogActions>
        </Dialog>
