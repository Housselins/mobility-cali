...
export const Banner = () => {
  ...
  const handleDeleteimage = async () => {
    ...
    try {
      await axios.delete(`${process.env.coreApi}/banner/${selectedBannerId}`);
      setBanners(
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post(`${process.env.coreApi}/banner`, formValues);
      await getBanners();
    } catch (error) {
      console.log(error);
    } finally {
  ...
  const getBanners = async () => {
    try {
      const response = await axios.get(`${process.env.coreApi}/banner`);
        ...
        <div className="w-full carousel">
          {banners.map((banner: any, index) => (
            <div
              key={index}
              id={"slide" + index}
              className={`carousel-item relative w-full h-2/4 ${
